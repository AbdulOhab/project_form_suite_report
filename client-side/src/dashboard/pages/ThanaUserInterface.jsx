  ...
  return (
    <>
      ...
      <Dialog
        ...
        onClose={() => setDescriptionAlert(false)}
        maxWidth="sm"
        fullWidth
      >
        <DialogTitle
        ...
        >
          ...
          <IconButton onClick={() => setDescriptionAlert(false)} size="small">
            <Close />
          </IconButton>
        </DialogTitle>
        ...
        <Box sx={{ mb: 2 }}>
          {daysLeft(notice?.endDadeline) < 0 ? (
            <Chip
              ...
              variant="outlined"
              size="small"
              label={`নোটিশ প্রদানের সময় শেষ হয়েছে ${convertToBengaliNumber(
                Math.abs(daysLeft(notice?.endDadeline))
              )} দিন পূর্বে`}
              sx={{ fontWeight: "bold" }}
            />
          ) : (
