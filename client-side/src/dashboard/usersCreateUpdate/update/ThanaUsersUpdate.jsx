...
function ThanaUsersUpdate() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/get-thana-users-for-update/${id}`, {
      method: "POST",
  ...
  function updateHandler(e) {
    ...
    fetch(`${BASE_URL}/update-thana-users/${id}`, {
      ...
      body: JSON.stringify({ userId, userName, thanaCode, branchCode, zonalCode }),
    })
    ...
    <Paper elevation={0} sx={{ borderRadius: 0, minHeight: "80vh" }}>
      ...
      <Box sx={{ px: 2, my: 3, maxWidth: 500, mx: "auto" }}>
        <Box sx={{ display: "flex", justifyContent: "center", mb: 3 }}>
          <Chip label="Update Thana" color="info" sx={{ fontWeight: "bold", fontSize: "1rem" }} />
        </Box>
        ...
        <Paper variant="outlined" sx={{ p: 3 }}>
          <form onSubmit={updateHandler}>
            ...
            <TextField fullWidth type="text" inputMode="numeric" name="thanaCode" label="Thana Code"
              required value={thanaCode} onChange={(e) => setThanaCode(e.target.value)} size="small" sx={{ mb: 2 }} />
