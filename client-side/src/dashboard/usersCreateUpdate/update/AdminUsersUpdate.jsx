...
function AdminUsersUpdate() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/get-admin-users/${id}`, {
      method: "POST",
    ...
    <Paper elevation={0} sx={{ borderRadius: 0, minHeight: "80vh" }}>
      ...
      <Box sx={{ px: 2, my: 3, maxWidth: 500, mx: "auto" }}>
        <Box sx={{ display: "flex", justifyContent: "center", mb: 3 }}>
          <Chip label="Update Admin" color="info" sx={{ fontWeight: "bold", fontSize: "1rem" }} />
        </Box>
        ...
        <Paper variant="outlined" sx={{ p: 3 }}>
          <form onSubmit={updateHandler}>
            ...
            <TextField fullWidth type="email" name="email" label="Email"
              value={email} onChange={(e) => setEmail(e.target.value)} size="small" sx={{ mb: 2 }} />
